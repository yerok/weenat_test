...
from api.tests.test_utils import print_json
...
class FetchDataRawTest(APITestCase):
    ...
    def test_filter_by_datalogger_and_date_range(self) -> None:
        ...
        expected = [
            ...
            if since <= m.at.isoformat() <= before and m.datalogger_id == self.datalogger.id
        ]
        response = self.client.get(
            self.url, {'datalogger': str(self.datalogger.id), 'since': since, 'before': before})

        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), len(expected))

    def test_invalid_date_filter(self) -> None:
        response = self.client.get(self.url, {'datalogger': str(self.datalogger.id), 'since': 'not-a-date'})
        self.assertEqual(response.status_code, 400)
    ...
    def test_invalid_datalogger_uuid(self) -> None:
        response = self.client.get(self.url, {'datalogger': '00000000-0000-0000-0000-000000000000'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 0)
