...
from api.tests.test_utils import aggregate, print_json
...
class FetchDataSummaryTest(APITestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.measurements = list(Measurement.objects.filter(
            datalogger=cls.datalogger).order_by("at"))
    ...
    def test_summary_filter_by_datalogger(self) -> None:
        response = self.client.get(
            self.url, {'datalogger': str(self.datalogger.id)})
    ...
    def test_summary_filter_by_date_range(self) -> None:
        ...
        expected = [m for m in self.measurements if since <=
                    m.at.isoformat() <= before]
        response = self.client.get(
            self.url, {'since': since, 'before': before, 'datalogger': str(self.datalogger.id)})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), len(expected))

    def test_summary_invalid_span(self) -> None:
        response = self.client.get(self.url, {
            'datalogger': str(self.datalogger.id),
            'span': 'minute'
        })
        self.assertEqual(response.status_code, 400)
    ...
    def test_summary_hourly_aggregation(self) -> None:
        ...
        for expected_item in expected:
            ...
            self.assertTrue(
                found, f"Item not found in response: {expected_item}")
