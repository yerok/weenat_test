...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--dataloggers', type=int, default=3, help="Number of dataloggers")
        parser.add_argument('--measurements', type=int, default=50, help="Measurements per datalogger")

    def handle(self, *args, **options):
        num_dataloggers = options['dataloggers']
        num_measurements = options['measurements']

        self.stdout.write(f"Creating {num_dataloggers} dataloggers with {num_measurements} measurements each...")
